...
async function obtenerDatos(cargar, index) {
  ...
  if (cargar == precargado) {
    crearObjeto(
      productoCargar.data.articles[0].image,
      productoCargar.data.articles[0].name,
      productoCargar.data.articles[0].currency,
      productoCargar.data.articles[0].unitCost,
      0,
      1
    );
  } else {
    crearObjeto(
      productoCargar.data.images[0],
      productoCargar.data.name,
      productoCargar.data.currency,
      productoCargar.data.cost,
      index,
      productoCargar.data.id
    );
  }
}
...
function crearObjeto(foto, name, moneda, precio, index, idp) {
  ...
  Ccantidad.innerHTML = `<div style="width: 60px;"><input type="number"  name="subtotal" id="subtotal${index}" required="" value="${cargarCantidad(
    index
  )}" min="1" class = "form-control" onchange="actualizarSubtotal(${index},${precio})" ></div>`;
  ...
  Psubtotal.innerHTML =
    precio * document.getElementById(`subtotal${index}`).value;
...
function borrarelemento(index) {
  document
    .getElementById("cartObjs")
    .removeChild(document.getElementById(`tr${index}`));
  let localsave = [];
...
function actualizarSubtotal(index, precio) {
  ...
  subtotalInicial.innerHTML =
    precio * document.getElementById(`subtotal${index}`).value;

  let diferencia = Math.abs(
    Number(localStorage.getItem("cantidad")) -
      Number(document.getElementById(`subtotal${index}`).value)
  );
  if (
    Number(localStorage.getItem("cantidad")) <
    Number(document.getElementById(`subtotal${index}`).value)
  ...
  for (let i = 1; i < arrayUnicos.length + 1; i++) {
    if (document.getElementById(`Psubtotal${i}`)) {
      if (document.getElementById(`Pcurrency${i}`).innerHTML === "UYU") {
        (sumaSubtotales +=
          parseInt(document.getElementById(`Psubtotal${i}`).innerHTML) / 41),
          02;
      } else {
        sumaSubtotales += parseInt(
          document.getElementById(`Psubtotal${i}`).innerHTML
        );
      }
    }
...
function actualizarCostoEnvio(porcentaje) {
  ...
  totalCostos.innerHTML =
    parseInt(NumeroCostosS.innerHTML) + parseInt(CostoDeEnvio.innerHTML);
}
...
form.addEventListener("submit", (e) => {
  ...
  if (valido < 4) {
    ...
    setTimeout(() => {
      document.getElementById("completado").classList.add("d-none");
    }, 1400);
  }
});
let valido = 0;
...
finalizarComp.addEventListener("click", () => {
  ...
  if (!tarjetaC.checkValidity() || !TranBan.checkValidity()) {
    ...
    ((tarjetaC.checkValidity() || TranBan.checkValidity()) &&
      NumCuenta.checkValidity() &&
      !NumCuenta.hasAttribute("disabled")) ||
    (numeroTarjeta.checkValidity() &&
      CodigoSeg.checkValidity() &&
      Venc.checkValidity() &&
      !Venc.hasAttribute("disabled"))
  ) {
